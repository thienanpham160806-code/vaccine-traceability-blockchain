...
export default function ProductDetailPage({ params }: PageProps) {
  ...
  const archiveSerial = async () => {
    ...
    try {
      ...
      toast.success("Đã ẩn serial khỏi web.");
      window.location.href = "/dashboard/products";
    } catch (err: unknown) {
      const msg = getApiErrorMessage(err, "Không thể ẩn serial.");
      setError(msg);
      toast.error(msg);
    }
  };
  ...
  const markAdministered = async () => {
    ...
    try {
      ...
      setMessage("Đã đánh dấu serial là đã tiêm. QR này không nên được dùng lại.");
      toast.success("Đã đánh dấu serial là đã tiêm.");
    } catch (err: unknown) {
      const msg = getApiErrorMessage(err, "Không thể đánh dấu đã tiêm.");
      setError(msg);
      toast.error(msg);
    } finally {
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          {product.status === "ADMINISTERED" ? (
            <p className="mt-2 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-semibold text-emerald-800">
              Serial này đã được xác nhận là đã tiêm. QR không còn hợp lệ để gắn lên sản phẩm khác.
            </p>
          ) : null}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {canReregisterOnChain && (
            ...
            >
              {isReregistering ? <ActionSpinner label="Đang xử lý..." /> : "Đăng ký lại on-chain"}
            </button>
          )}
          {canTransfer ? (
            <Link
          ...
          {canAdminister ? (
            <button
              ...
              type="button"
            >
              {isAdministering ? <ActionSpinner label="Đang xử lý..." /> : "Đánh dấu đã tiêm"}
            </button>
          ) : null}
          {canArchive ? (
            <button
      ...
      <div className="grid gap-6 xl:grid-cols-[1.4fr_0.8fr]">
        <section className="rounded-xl border border-zinc-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 grid gap-4 text-sm md:grid-cols-2">
            ...
            <div>
              <p className="font-semibold text-gray-700">Vị trí hiện tại</p>
