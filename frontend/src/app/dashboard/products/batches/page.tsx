    ...
    >
      <div className="min-w-0 flex-1">
        <div className="mb-1 flex flex-wrap items-center gap-2">
          ...
          <span className={`rounded-full border px-2.5 py-0.5 text-[11px] font-semibold ${isRecalled ? "border-red-200 bg-red-50 text-red-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border bg-white p-4 shadow-sm">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-zinc-800">{t("Tất cả lô hàng")}{batches.length > 0 ? <span className="ml-2 font-normal text-zinc-400">({batches.length})</span> : null}</h2>
        </div>
        <div className="flex flex-wrap gap-2">
          <select className="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
          ...
          <button onClick={() => queryClient.invalidateQueries({ queryKey: ["batches"] })} className="flex items-center gap-1.5 rounded-lg border border-zinc-200 px-3 py-2 text-sm font-semibold text-zinc-600 hover:bg-zinc-50">
      ...
      {isLoading ? (
        <div className="space-y-2">{[1, 2, 3].map((item) => <div key={item} className="h-20 animate-pulse rounded-xl bg-zinc-100" />)}</div>
      ) : filteredBatches.length === 0 ? (
        <div className="flex flex-col items-center rounded-2xl border border-dashed border-zinc-300 py-14 text-center">
          <Boxes className="mb-3 h-8 w-8 text-zinc-300" />
          ...
          <Link href="/dashboard/products/register" className="mt-4 rounded-lg border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-700 hover:bg-blue-100">{t("Đăng ký sản phẩm")}</Link>
        </div>
      ) : (
        <div className="space-y-2">{filteredBatches.map((batch) => <BatchRow key={batch.id || batch.batchHash} batch={batch} t={t} />)}</div>
      )}
    </div>
  );
}
